...
def gerar_pdf(apostas_registradas):
    ...
    if not apostas_registradas:
        ...
        for aposta in apostas_registradas:
            elementos.append(Paragraph(f"Aposta {aposta['id']} - {aposta['jogo']}", styles["Heading2"]))
            elementos.append(Paragraph(f"Cartões: {aposta['mercado_cartao']} | Odd: {numero(aposta['odd_cartao'])}", styles["Normal"]))
            elementos.append(Paragraph(f"Escanteios: {aposta['mercado_escanteio']} | Odd: {numero(aposta['odd_escanteio'])}", styles["Normal"]))
...
if resultado["existe_arbitragem"]:
    st.success("A distribuição acima cobre os três resultados possíveis usando as odds finais combinadas.")
else:
...
if st.session_state.apostas_registradas:
    ...
    st.info("Nenhuma aposta registrada ainda. Configure a aposta acima e clique em 'Adicionar aposta'.")
